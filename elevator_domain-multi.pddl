...
(lift-at ?floor - floor ?e - elevator)
               ...
               (forall (?p - passenger) 
                  (when (and (boarded ?p) 
                        ...
                        (and (not (boarded ?p)) 
                             (served  ?p))))
               (forall (?p - passenger)                
                   (when (and (origin ?p ?f ?e) (not (served ?p)))
                              (boarded ?p)))))
...
(:action up
  ...
  :precondition (and (lift-at ?f1 ?e1) (above ?f1 ?f2))
  :effect (and (lift-at ?f2 ?e1) (not (lift-at ?f1 ?e1))))
...
(:action down
  :parameters (?f1 - floor ?f2 - floor ?e1 - elevator)
  :precondition (and (lift-at ?f1 ?e1) (above ?f2 ?f1))
  :effect (and (lift-at ?f2 ?e1) (not (lift-at ?f1 ?e1))))
)
